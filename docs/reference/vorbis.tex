...
\VAR{Length Count} values.
\begin{align*}
\text{Length Count}_i &= \lfloor\log_2 (\text{Remaining Entries}_i)\rfloor + 1
\intertext{For example, given a \VAR{Total Entries} value of 8 and an
\VAR{Initial Length} value of 2:}
\text{Length Count}_0 &= \lfloor\log_2 8\rfloor + 1 = 4 \text{ bits}
\end{align*}
...
value of the length:
\begin{equation*}
\text{Length Count}_1 = \lfloor\log_2 6\rfloor + 1 = 3 \text{ bits}
\end{equation*}
...
\begin{equation*}
\text{Length Count}_2 = \lfloor\log_2 4\rfloor + 1 = 3 \text{ bits}
\end{equation*}
