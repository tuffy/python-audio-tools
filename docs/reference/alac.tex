...
\While{$PCM~frame~count > 0$}{
  decode ALAC frame to 1 or more PCM frames\;
  deduct ALAC frame's samples from stream's $PCM~frame~count$\;
...
\VAR{atom type} an ASCII string and \VAR{atom data}
may contain one or more sub-atoms.
\end{wrapfigure}
