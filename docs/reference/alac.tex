...
\eIf(\tcc*[f]{raw, uncompressed frame}){$not~compressed = 1$}{
  \For{i = 0 \emph{\KwTo}sample count}{
    \For{c = 0 \emph{\KwTo}channel count}{
      ...
      $\text{channel}_{i~c} \leftarrow$ \READ (bits per sample) signed bits\;
    }
  }
  ...
  \If{$uncompressed~LSBs > 0$}{
    \For{i = 0 \emph{\KwTo}sample count}{
      \For{c = 0 \emph{\KwTo}channel count}{
        $\text{LSBs}_{i~c} \leftarrow$ \READ ($uncompressed~LSBs \times 8$) signed bits\;
      }
    }
  }
  \For{c = 0 \emph{\KwTo}channel count}{
    $\text{residual block}_c \leftarrow$ read residual block\;
  ...
  \If{$uncompressed~LSBs > 0$}{
    prepend each $\text{LSBs}_{i~c}$ to its corresponding $\text{channel}_{i~c}$ sample\;
  }
}
\Return channel data\;
\EALGORITHM
