...
\eIf{$\HASSAMPLECOUNT = 0$}{
  \SAMPLECOUNT $\leftarrow$ samples per frame from \texttt{alac} atom\;
}{
  \SAMPLECOUNT $\leftarrow$ \READ 32 unsigned bits\;
}
\eIf(\tcc*[f]{raw, uncompressed frame}){$\NOTCOMPRESSED = 1$}{
  ...
  \For{$c \leftarrow 0$ \emph{\KwTo}\CHANNELCOUNT}{
    $\text{\SUBFRAMEHEADER}_c \leftarrow$ read subframe header\;
  }
  ...
  \For{$c \leftarrow 0$ \emph{\KwTo}\CHANNELCOUNT}{
    $\text{\RESIDUALS}_c \leftarrow$ read residual block where sample size is $\text{bits per sample} - (\UNCOMPRESSEDLSBS \times 8) + (\text{\CHANNELCOUNT} - 1)$\;
    decode $\text{\SUBFRAME}_c$ from $\text{\SUBFRAMEHEADER}_c$ and $\text{\RESIDUALS}_c$\;
  }
  \eIf{$\CHANNELCOUNT = 2$}{
    decorrelate $\text{\SUBFRAME}_0$ and $\text{\SUBFRAME}_1$ to $\text{\CHANNEL}_0$ and $\text{\CHANNEL}_1$\newline according to \INTERLACINGSHIFT and \INTERLACINGLEFTWEIGHT\;
  }{
    \For{$c \leftarrow 0$ \emph{\KwTo}\CHANNELCOUNT}{
...
$\RICEMODIFIER \leftarrow$ \READ 3 unsigned bits\tcc*[r]{unusued}
...
\textsf{notcompressed} & $\leftarrow$ & \texttt{0} \\
...
\For{i $\leftarrow \text{\COEFFCOUNT} + 1$ \emph{\KwTo}$\text{\SAMPLECOUNT}$}{
  ...
  $\text{\SAMPLE}_i \leftarrow \left\lfloor\frac{\text{\QLPSUM}_i + 2 ^ \text{\QLPSHIFT - 1}}{2 ^ \text{\QLPSHIFT}}\right\rfloor + \text{\RESIDUAL}_i + \text{\BASE}_i$\;
  \BlankLine
  \uIf(\tcc*[f]{modify QLP coefficients}){$\text{\RESIDUAL}_i > 0$}{
    \For{$j \leftarrow 0$ \emph{\KwTo}$\text{\COEFFCOUNT}$}{
      $\DIFF \leftarrow \text{\BASE}_i - \text{Samples}_{i - \COEFFCOUNT + j}$\;
...
\eIf{$\text{\LEFTWEIGHT} > 0$}{
  ...
  $\LEFT \leftarrow \text{\SUBFRAME}_0$\;
  $\RIGHT \leftarrow \text{\SUBFRAME}_1$\;
}
\Return $(\LEFT~,~\RIGHT)$\;
\EALGORITHM
...
\SetKwData{LSB}{LSB}
$0 \rightarrow$ \WRITE 16 unsigned bits\tcc*[r]{unusued}
\eIf{$\text{\PCMCOUNT} \neq \text{encoding parameter's \BLOCKSIZE}$}{
  $1 \rightarrow$ \WRITE 1 unsigned bit\;
}{
  $0 \rightarrow$ \WRITE 1 unsigned bit\;
...
write subframe header with $\text{\QLPCOEFF}_0$\;
...
\begin{figure}[h]
  \includegraphics{figures/alac/noninterlaced_frame.pdf}
\end{figure}
...
\BlankLine
correlate $(\text{\CHANNEL}_0~,~\text{\CHANNEL}_1)$ to $(\text{\CORRELATED}_0~,~\text{\CORRELATED}_1)$\newline with \INTERLACINGSHIFT and \INTERLACINGLEFTWEIGHT\;
\BlankLine
...
write subframe header with $\text{\QLPCOEFF}_0$\;
write subframe header with $\text{\QLPCOEFF}_1$\;
\BlankLine
\If{$\text{\UNCOMPRESSEDLSB} > 0$}{
  \For{$i \leftarrow 0$ \emph{\KwTo}\PCMCOUNT}{
    $\text{LSB}_i \rightarrow$ \WRITE $(\text{\UNCOMPRESSEDLSB} \times 8)$ unsigned bits\;
...
\begin{figure}[h]
  \includegraphics{figures/alac/interlaced_frame.pdf}
\end{figure}
...
\SetKwData{SAMPLESIZE}{sample size}
\SetKwData{RESIDUAL}{residual}
window signed integer \SAMPLES to floating point \WINDOWED values\;
autocorrelate \WINDOWED to 9 floating point \AUTOCORRELATION values\;
\eIf{$\text{\AUTOCORRELATION}_0 \neq 0.0$}{
  compute 8 lists of floating point \LPCOEFF from \AUTOCORRELATION\;
  quantize $\text{\LPCOEFF}_3$ to a list of integer $\text{\QLPCOEFF}_3$ at order 4\;
  quantize $\text{\LPCOEFF}_7$ to a list of integer $\text{\QLPCOEFF}_7$ at order 8\;
  calculate $\text{\RESIDUAL}_3$ from $\text{\QLPCOEFF}_3$ and \SAMPLES\;
  calculate $\text{\RESIDUAL}_7$ from $\text{\QLPCOEFF}_7$ and \SAMPLES\;
  encode $\text{Residual Block}_3$ from $\text{\RESIDUAL}_3$ with \SAMPLESIZE and encoding parameters\;
  encode $\text{Residual Block}_7$ from $\text{\RESIDUAL}_7$ with \SAMPLESIZE and encoding parameters\;
  \eIf{$\LEN(\text{Residual Block}_3) < (\LEN(\text{Residual Block}_7) + 64~bits)$}{
    \Return ($\text{\QLPCOEFF}_3~,~\text{Residual Block}_3$)\;
  }{
    \Return ($\text{\QLPCOEFF}_7~,~\text{Residual Block}_7$)\;
  }
}(\tcc*[f]{all samples are 0}){
  \QLPCOEFF $\leftarrow$ \texttt{[0, 0, 0, 0]}\;
  calculate $\text{\RESIDUAL}$ from \QLPCOEFF and \SAMPLES\;
  encode $\text{Residual Block}$ from $\text{\RESIDUAL}$ with \SAMPLESIZE and encoding parameters\;
  \Return ($\text{\QLPCOEFF}~,~\text{Residual Block}$)\;
}
\EALGORITHM
}
...
\SetKwData{RESIDUAL}{residual}
\SetKwData{SAMPLE}{sample}
\SetKwData{BASESAMPLE}{base sample}
\SetKwData{QLPSUM}{QLP sum}
\SetKwData{DIFF}{diff}
\SetKwData{SSIGN}{sign}
$\text{\RESIDUAL}_0 \leftarrow \text{\SAMPLE}_0$\;
\For{$i \leftarrow 1$ \emph{\KwTo}$\COEFFCOUNT + 1$}{
  $\text{\RESIDUAL}_i \leftarrow \text{\SAMPLE}_i - \text{\SAMPLE}_{i - 1}$
}
\For{i $\leftarrow \text{\COEFFCOUNT} + 1$ \emph{\KwTo}\SAMPLECOUNT}{
  $\text{\BASESAMPLE}_i \leftarrow \text{\SAMPLE}_{i - \COEFFCOUNT - 1}$\;
  $\text{\QLPSUM}_i \leftarrow \overset{\COEFFCOUNT - 1}{\underset{j = 0}{\sum}} \text{\COEFF}_j \times (\text{\SAMPLE}_{i - j - 1} - \text{\BASESAMPLE}_i)$\;
  $\text{\ERROR} \leftarrow \text{\SAMPLE}_i - \left(\left\lfloor\frac{\text{\QLPSUM}_i + 2 ^ 8}{2 ^ 9}\right\rfloor + \text{\BASESAMPLE}_i\right)$\;
  $\text{\RESIDUAL}_i \leftarrow \text{\ERROR}$\;
  \BlankLine
  \uIf(\tcc*[f]{modify QLP coefficients}){$\text{\ERROR} > 0$}{
    \For{$j \leftarrow 0$ \emph{\KwTo}$\text{\COEFFCOUNT}$}{
      $\DIFF \leftarrow \text{\BASESAMPLE}_i - \text{\SAMPLE}_{i - \COEFFCOUNT + j}$\;
      $\SSIGN \leftarrow \SIGN(\DIFF)$\;
      $\text{\COEFF}_{\COEFFCOUNT - j - 1} \leftarrow \text{\COEFF}_{\COEFFCOUNT - j - 1} - \SSIGN$\;
      $\text{\ERROR} \leftarrow \text{\ERROR} - \left\lfloor\frac{\DIFF \times \SSIGN}{2 ^ 9}\right\rfloor \times (j + 1)$\;
      \If{$\text{\ERROR} \leq 0$}{
        \BREAK\;
      }
    }
...
\ALGORITHM{a list of signed residual values, sample size; initial history, history multiplier, maximum K from encoding options}{a block of residual data, or a \textit{residual overflow} exception}
\SetKwData{RESIDUAL}{residual}
...
\While{$i < \text{residual count}$}{
  ...
  \If{$\text{\UNSIGNED}_i \geq 2 ^ \text{\SAMPLESIZE}$}{
  ...
  $\WRITERESIDUAL(\text{\UNSIGNED}_i - \SIGNMODIFIER~,~\kappa~,~\text{\SAMPLESIZE})$\;
  $\SIGNMODIFIER \leftarrow 0$\;
  \BlankLine
...
\eIf{$\MSB > 8$}{
  $\texttt{0x1FF} \rightarrow$ \WRITE 9 unsigned bits\;
  $\text{\UNSIGNED} \rightarrow$ \WRITE (sample size) unsigned bits\;
}{
  $\MSB \rightarrow$ \WUNARY with stop bit 0\;
  \If{$\kappa > 1$}{
    \eIf{$\LSB > 0$}{
      $\LSB + 1 \rightarrow$ \WRITE $\kappa$ unsigned bits\;
    }{
      $0 \rightarrow$ \WRITE $(\kappa - 1)$ unsigned bits\;
    }
  }
}
\Return encoded residual data\;
\EALGORITHM
