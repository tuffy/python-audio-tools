...
Step 2. & j &=& 0 \\
Step 3. & v &=& \text{samples}_{i - \text{count} - 1} - \text{samples}_{i - \text{count} + j} \\
Step 4. & sign &=& \begin{cases}\text{sign}(v) & \text{if original sign} > 0 \\
-\text{sign}(v) & \text{if original sign} \leq 0\end{cases} \\
Step 5. & \text{coefficients}_{\text{count} - j - 1} &=& \text{coefficients}_{\text{count} - j - 1} - sign \\
...
j &= 0 \\
v &= \text{samples}_0 - \text{samples}_{1} = 16 - 17 = \textbf{-1} \\
sign &= \text{sign}(-1) = \textbf{-1} \\
\text{coefficients}_3 &= \text{coefficients}_3 - -1 = 122 + 1 = \textbf{123} \\
residual &= residual - \left( \left\lfloor\frac{-1 \times -1}{2 ^ 9}\right\rfloor \times (0 + 1)\right) = 1 - (0 \times 1) = \textbf{1} \\
j &= 1 \\
v &= \text{samples}_0 - \text{samples}_{2} = 16 - 26 = \textbf{-10} \\
sign &= \text{sign}(-10) = \textbf{-1} \\
\text{coefficients}_2 &= \text{coefficients}_2 - -1 = 107 + 1 = \textbf{108} \\
residual &= residual - \left( \left\lfloor\frac{-10 \times -1}{2 ^ 9}\right\rfloor \times (1 + 1)\right) = 1 - (0 \times 2) = \textbf{1} \\
j &= 2 \\
v &= \text{samples}_0 - \text{samples}_{3} = 16 - 25 = \textbf{-9} \\
sign &= \text{sign}(-9) = \textbf{-1} \\
\text{coefficients}_1 &= \text{coefficients}_1 - -1 = -766 + 1 = \textbf{-765} \\
residual &= residual - \left( \left\lfloor\frac{-9 \times -1}{2 ^ 9}\right\rfloor \times (2 + 1)\right) = 1 - (0 \times 3) = \textbf{1} \\
j &= 3 \\
v &= \text{samples}_0 - \text{samples}_{4} = 16 - 25 = \textbf{-8} \\
sign &= \text{sign}(-8) = \textbf{-1} \\
\text{coefficients}_0 &= \text{coefficients}_0 - -1 = 1122 + 1 = \textbf{1123} \\
...
%this is largely a cut-&-paste from the decoding section
...
\end{align*}
Thus, our list of coefficients for $\text{Residual}_6$ are now
...
\end{align*}
%another cut-&-paste from the decoding section
\begin{table}[h]
\begin{tabular}{|c| >{$}l<{$} >{$}r<{$} | >{$}r<{$} |}
\hline
j & & \text{Val}_j & \text{Sign}_j \\
