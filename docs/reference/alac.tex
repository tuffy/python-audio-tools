...
\For{$i \leftarrow 0$ \emph{\KwTo}coefficient count}{
...
\For{$i \leftarrow 1$ \emph{\KwTo}$\text{coefficient count} + 1$}{
  $\text{Sample}_i \leftarrow \text{Residual}_{i} + \text{Sample}_{i - 1}$\;
}
\BlankLine
\For{i $\leftarrow \text{coefficient count} + 1$ \emph{\KwTo}$\text{sample count}$}{
  $\text{Base Sample}_i \leftarrow \text{Sample}_{i - coeff.~count - 1}$\;
  $\text{QLP Sum}_i \leftarrow \overset{coeff.~count - 1}{\underset{j = 0}{\sum}} \text{QLP Coefficient}_j \times (\text{Sample}_{i - j - 1} - \text{Base Sample}_i)$\;
  $\text{Sample}_i \leftarrow \left\lfloor\frac{\text{QLP Sum}_i + 2 ^ \text{QLP shift needed - 1}}{2 ^ \text{QLP shift needed}}\right\rfloor + \text{Residual}_i + \text{Base Sample}_i$\;
  \BlankLine
  \uIf(\tcc*[f]{modify QLP coefficients}){$\text{Residual}_i > 0$}{
    \For{$j \leftarrow 0$ \emph{\KwTo}$\text{coefficient count}$}{
...
compute $\text{QLP coefficients}_0$ and $\text{Residual}_0$ for $\text{Channel}_0$ with \SAMPLESIZE\;
write subframe header with $\text{QLP coefficients}_0$\;
\If{$\text{uncompressed LSBs} > 0$}{
  \For{$i \leftarrow 0$ \emph{\KwTo}\PCMCOUNT}{
    $\text{LSB}_i \rightarrow$ \WRITE $(\text{uncompressed LSBs} \times 8)$ unsigned bits\;
  }
}
...
$\text{interlacing shift} \rightarrow$ \WRITE 8 unsigned bits\;
$\text{interlacing leftweight} \rightarrow$ \WRITE 8 unsigned bits\;
...
\For{$i \leftarrow 1$ \emph{\KwTo}coefficient count + 1}{
  $\text{Residual}_i \leftarrow \text{Sample}_i - \text{Sample}_{i - 1}$
}
\For{i $\leftarrow \text{coefficient count} + 1$ \emph{\KwTo}sample count}{
  $\text{Base Sample}_i \leftarrow \text{Sample}_{i - coeff.~count - 1}$\;
  $\text{QLP Sum}_i \leftarrow \overset{coeff.~count - 1}{\underset{j = 0}{\sum}} \text{QLP Coefficient}_j \times (\text{Sample}_{i - j - 1} - \text{Base Sample}_i)$\;
  $\text{\ERROR} \leftarrow \text{Sample}_i - \left(\left\lfloor\frac{\text{QLP Sum}_i + 2 ^ 8}{2 ^ 9}\right\rfloor + \text{Base Sample}_i\right)$\;
  ...
  \uIf(\tcc*[f]{modify QLP coefficients}){$\text{\ERROR} > 0$}{
    \For{$j \leftarrow 0$ \emph{\KwTo}$\text{coefficient count}$}{
      ...
      $sign \leftarrow \SIGN(diff)$\;
      $\text{QLP Coefficient}_{coeff.~count - j - 1} \leftarrow \text{QLP Coefficient}_{coeff.~count - j - 1} - sign$\;
      $\text{\ERROR} \leftarrow \text{\ERROR} - \left\lfloor\frac{diff \times sign}{2 ^ 9}\right\rfloor \times (j + 1)$\;
      \If{$\text{\ERROR} \leq 0$}{
        \BREAK\;
      }
    }
...
\noindent
\begin{tabular}{r||r|r|>{$}r<{$}|>{$}r<{$}|>{$}r<{$}}
$i$ & $\text{Sample}_i$ & $\text{Base}_i$ & \text{QLP Sum}_i & \text{Residual}_i & \text{QLP Coeff.}_{(i + 1)~j} \\
\hline
0 & 0 & & & 0 \\
1 & 16 & & & 16 - 0 = 16 \\
2 & 32 & & & 32 - 16 = 16 \\
...
$\text{coefficient count} \rightarrow$ \WRITE 5 unsigned bits\;
\ForEach{QLP coefficient \IN QLP coefficients}{
  $\text{QLP coefficient} \rightarrow$ \WRITE 16 signed bits\;
}
\EALGORITHM
\begin{figure}[h]
\includegraphics{figures/alac/subframe_header.pdf}
