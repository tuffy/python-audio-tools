...
\eIf{$\CHANNELCOUNT = 2$ \AND (try mid-side \OR try adaptive mid-side)}{
  \begin{tabular}{rcl}
    $(\AVERAGE~,~\DIFFERENCE)$ & $\leftarrow$ & \hyperref[flac:calc_midside]{calculuate mid-side of $\text{\CHANNEL}_0$ and $\text{\CHANNEL}_1$} \\
  ...
  \hyperref[flac:write_frame_header]{write frame header with channel assigment $\CHANNELCOUNT - 1$}\;
  \For{$c \leftarrow 0$ \emph{\KwTo}\CHANNELCOUNT}{
