...
\ALGORITHM{the frame's block size and bits per sample, the subframe's channel assignment and the FLAC file stream}{decoded PCM samples}
...
\ALGORITHM{the frame's block size, the subframe's bits per sample and the FLAC file stream}{decoded PCM samples}
...
\ALGORITHM{the frame's block size and predictor order, the subframe's bits per sample and the FLAC file stream}{decoded PCM samples}
\SetKwData{BPS}{bits per sample}
\SetKwData{ORDER}{order}
...
\Switch{\ORDER}{
  ...
  \uCase{1} {
    \For{i = 1 \emph{\KwTo}\BLOCKSIZE}{
      $sample_i \leftarrow sample_{i - 1} + residual_i$
    }
  }
  \uCase{2} {
    \For{i = 2 \emph{\KwTo}\BLOCKSIZE}{
      $sample_i \leftarrow (2 \times sample_{i - 1}) - sample_{i - 2} + residual_i$
    }
  }
  \uCase{3} {
    \For{i = 3 \emph{\KwTo}\BLOCKSIZE}{
      $sample_i \leftarrow (3 \times sample_{i - 1}) - (3 \times sample_{i - 2}) + sample_{i - 3} + residual_i$
    }
  }
  \Case{4} {
    \For{i = 0 \emph{\KwTo}\BLOCKSIZE}{
      $sample_i \leftarrow (4 \times sample_{i - 1}) - (6 \times sample_{i - 2}) + (4 \times sample_{i - 3}) - sample_{i - 4} + residual_i$
    }
  }
}
\Return samples\;
