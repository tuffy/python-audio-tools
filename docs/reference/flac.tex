...
\begin{figure}[h]
  \includegraphics{figures/flac/metadata-blocks.pdf}
\end{figure}
\par
\noindent
...
\begin{figure}[h]
  \includegraphics{figures/flac/streaminfo-example.pdf}
\end{figure}
\begin{tabular}{rrl}
...
\subsection{PADDING}
\begin{figure}[h]
  \includegraphics{figures/flac/padding.pdf}
\end{figure}
\par
\noindent
...
\begin{figure}[h]
  \includegraphics{figures/flac/seektable-example.pdf}
\end{figure}
\begin{table}[h]
{\relsize{-1}
\begin{tabular}{r|rrr}
...
\begin{figure}[h]
  \includegraphics{figures/flac/vorbiscomment-example.pdf}
\end{figure}
\begin{table}[h]
\begin{tabular}{rl}
...
%% \begin{figure}[h]
%%   \includegraphics{figures/flac/cuesheet-example.pdf}
%% \end{figure}
...
\begin{figure}[h]
  \includegraphics{figures/flac/picture-example.pdf}
\end{figure}
\begin{table}[h]
{\relsize{-1}
  \begin{tabular}{rl}
...
\eIf{all samples are the same}{
  ...
  \eIf(\tcc*[f]{from encoding parameters}){maximum LPC order $ > 0$}{
    ...
    \hyperref[flac:compute_lpc_params]{compute LPC parameters from \SAMPLE}\;
...
\ALGORITHM{signed subframe samples, subframe's bits per sample, wasted BPS}{a VERBATIM subframe}
\SetKwData{BLOCKSIZE}{block size}
\SetKwData{SAMPLE}{sample}
\SetKwData{BPS}{subframe's BPS}
\SetKwData{WASTEDBPS}{wasted BPS}
...
\For{$o \leftarrow \text{\MINPORDER}$ \emph{\KwTo}(\MAXPORDER + 1)}{
  \eIf{$(\BLOCKSIZE \bmod 2^{o}) = 0$}{
    $\left.\begin{tabular}{r}
      ...
      $\text{\PARTITION}_o$ \\
      $\text{\PSIZE}_o$ \\
    \end{tabular}\right\rbrace \leftarrow$ \hyperref[flac:write_residual_partition]{encode residual partition(s) with order $o$}\;
  }{
    \BREAK\;
  }
}
...
Add estimating best order example here.
