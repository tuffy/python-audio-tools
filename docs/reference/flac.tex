...
Given the subframe bytes\footnote{Decoding the residual block is explained on page \pageref{residual_decoding_example}}:
\begin{figure}[h]
...
\begin{tabular}{rcr}
$\text{sample}_0$ & $\leftarrow$ & 37 \\
$\text{sample}_1$ & $\leftarrow$ & 37 - 2 = 35 \\
$\text{sample}_2$ & $\leftarrow$ & 35 + 3 = 38 \\
$\text{sample}_3$ & $\leftarrow$ & 38 - 1 = 37 \\
$\text{sample}_4$ & $\leftarrow$ & 37 - 5 = 32 \\
$\text{sample}_5$ & $\leftarrow$ & 32 + 1 = 33 \\
$\text{sample}_6$ & $\leftarrow$ & 33 - 5 = 27 \\
$\text{sample}_7$ & $\leftarrow$ & 27 + 4 = 31 \\
$\text{sample}_8$ & $\leftarrow$ & 31 - 2 = 29 \\
$\text{sample}_9$ & $\leftarrow$ & 29 - 3 = 26 \\
$\text{sample}_{10}$ & $\leftarrow$ & 26 + 1 = 27 \\
\end{tabular}
...
QLP precision $\leftarrow$ \READ 4 unsigned bits\;
QLP shift needed $\leftarrow$ \MAX(\READ 5 signed bits, 0)\;
\For{i = 0 \emph{\KwTo}\ORDER}{
  $QLP~coefficient_i \leftarrow$ \READ (QLP precision + 1) signed bits\;
}
\RESIDUALS $\leftarrow$ read residual block with frame's \BLOCKSIZE and subframe's \ORDER\;
...
\ALGORITHM{the residual block's coding method, the partition's residual count, the FLAC file stream}{decoded signed residual values}
  ...
  \For{i = 0 \emph{\KwTo}partition residual count} {
    ...
    \eIf{unsigned is even}{
      $residual_i \leftarrow \lfloor unsigned \div 2 \rfloor$\;
    }{
...
$\text{residual}_1$ & $\leftarrow$ & $\lfloor 6 \div 2\rfloor = 3$ &
$\text{residual}_6$ & $\leftarrow$ & $\lfloor 8 \div 2\rfloor = 4$ \\
\hline
...
$\text{residual}_4$ & $\leftarrow$ & $\lfloor 6 \div 2\rfloor = 1$ &
$\text{residual}_9$ & $\leftarrow$ & $\lfloor 6 \div 2\rfloor = 1$ \\
\end{tabular}
}
\par
\noindent
