...
\begin{tabular}{rcl}
$\ERROR_0$ &$\leftarrow$ & $\AUTOCORRELATION_0$ \\
$\kappa_0$ &$\leftarrow$ & $ \AUTOCORRELATION_1 \div \ERROR_0$ \\
$\LPCOEFF_{0~0}$ &$\leftarrow$ & $ \kappa_0$ \\
$\ERROR_1$ &$\leftarrow$ & $ \ERROR_0 \times (1 - {\kappa_0} ^ 2)$ \\
\end{tabular}\;
\For{i = 1 \emph{\KwTo}maximum LPC order + 1}{
  ...
  \BlankLine
  $\ERROR_{i + 1} \leftarrow \ERROR_i \times (1 - {\kappa_i}^2)$\;
}
\Return list of LP coefficient lists, and error value list\;
\EALGORITHM
}
...
Our inputs are $M$, the maximum LPC order plus 1, and $r$ autocorrelation
