...
\For{$p \leftarrow 0$ \emph{\KwTo}$2 ^ {\text{partition order}}$}{
  \eIf{p = 0}{
...
\ALGORITHM{the residual block's coding method, the partition's residual count, the FLAC file stream}{decoded signed residual values}
  \uIf{coding method = 0}{
    ...
    \If(\tcc*[f]{handle unencoded residuals}){Rice parameter = 15}{
      ...
      \For{$i \leftarrow 0$ \emph{\KwTo}partition residual count}{
...
\uIf(\tcc*[f]{independent}){$0 \leq encoded~channels \leq 7$}{
 channel count $\leftarrow encoded~channels + 1$\;
  \For{$i \leftarrow 0$ \emph{\KwTo}block size}{
    \For{$j \leftarrow 0$ \emph{\KwTo}channel count}{
...
$\text{default block size} \rightarrow$ \WRITE 16 unsigned bits\tcc*[r]{minimum block size}
$\text{default block size} \rightarrow$ \WRITE 16 unsigned bits\tcc*[r]{maximum block size}
$0 \rightarrow$ \WRITE 24 unsigned bits\tcc*[r]{minimum frame size}
$0 \rightarrow$ \WRITE 24 unsigned bits\tcc*[r]{maximum frame size}
$\text{stream's sample rate} \rightarrow$ \WRITE 20 unsigned bits\;
$\text{stream's channels} - 1 \rightarrow$ \WRITE 3 unsigned bits\;
$\text{stream's bits per sample} - 1 \rightarrow$ \WRITE 5 unsigned bits\;
...
$\text{encoded~block~size} \rightarrow$ \WRITE 4 unsigned bits\;
$\text{encoded~sample~rate} \rightarrow$ \WRITE 4 unsigned bits\;
$\text{frame's channel assignment} \rightarrow$ \WRITE 4 unsigned bits\;
$encoded~bps \rightarrow$ \WRITE 3 unsigned bits\;
$0 \rightarrow$ \WRITE 1 unsigned bit\;
$\text{frame number} \rightarrow$ \WRITE as UTF-8 encoded value\;
\uIf{encoded block size = 6}{
  $(\text{block size} - 1) \rightarrow$ \WRITE 8 unsigned bits\;
}
\ElseIf{encoded block size = 7}{
  $\text{block size} - 1 \rightarrow$ \WRITE 16 unsigned bits\;
}
\uIf{encoded sample rate = 12}{
  $\text{sample rate} \div 1000 \rightarrow$ \WRITE 8 unsigned bits\;
}
\uElseIf{encoded sample rate = 13}{
  $\text{sample rate} \rightarrow$ \WRITE 16 unsigned bits\;
}
\ElseIf{encoded sample rate = 14}{
  $\text{sample rate} \div 10 \rightarrow$ \WRITE 16 unsigned bits\;
}
$\text{header CRC8} \rightarrow$ \WRITE 8 unsigned bits\;
\EALGORITHM
...
\eIf{value $\leq 127$}{
  $\text{value} \rightarrow$ \WRITE 8 unsigned bits\;
}{
  \uIf{value $\leq 2047$}{
  ...
  $\text{total bytes} \rightarrow$ \WUNARY with stop bit 0\;
  $\lfloor \text{value} \div 2 ^ \text{shift} \rfloor \rightarrow$ \WRITE (7 - total bytes) unsigned bits\tcc*[r]{initial value}
  shift $\leftarrow$ shift - 6\;
  \While{$shift \geq 0$}{
    $2 \rightarrow$ \WRITE 2 unsigned bits\tcc*[r]{continuation header}
    $\lfloor \text{value} \div 2 ^ \text{shift} \rfloor \bmod 64 \rightarrow$ \WRITE 6 unsigned bits\tcc*[r]{continuation bits}
    shift $\leftarrow$ shift - 6\;
  }
}
\EALGORITHM
...
$\text{sample} \rightarrow$ \WRITE (bits per sample) signed bits\;
...
\For(\tcc*[f]{order 0}){$i \leftarrow 0$ \emph{\KwTo}sample count}{
...
\For(\tcc*[f]{order 1-4}){$order \leftarrow 1$ \emph{\KwTo}5}{
  \For{$i \leftarrow 0$ \emph{\KwTo}sample count - order}{
...
$\text{subframe order} \rightarrow$ \WRITE 3 unsigned bits\;
\eIf{$wasted~BPS > 0$}{
  $1 \rightarrow$ \WRITE 1 unsigned bit\;
  $\text{wasted BPS} - 1 \rightarrow$ \WUNARY with stop bit 1\;
}{
  $0 \rightarrow$ \WRITE 1 unsigned bit\;
}
\For(\tcc*[f]{warm-up samples}){$i \leftarrow 0$ \emph{\KwTo}subframe order}{
...
\For{$\PORDER \leftarrow \text{minimum partition order}$ \emph{\KwTo}(maximum partition order + 1)}{
  \If{block size equally divisible into $2^{\PORDER}$ partitions}{
    ...
    \For{$p \leftarrow 0$ \emph{\KwTo}$2 ^ {\PORDER}$} {
      \eIf{p = 0}{
...
\eIf{any $\text{\RICE}_{\BPORDER} > 14$}{
  coding method $\leftarrow 1$\;
}{
  coding method $\leftarrow 0$\;
}
$\text{coding method} \rightarrow$ \WRITE 2 unsigned bits\;
$\BPORDER \rightarrow$ \WRITE 4 unsigned bits\;
\For{$p \leftarrow 0$ \emph{\KwTo}$2 ^ {\BPORDER}$} {
  \eIf{coding method = 0}{
    $\text{\RICE}_{(\BPORDER)~p} \rightarrow$ \WRITE 4 unsigned bits\;
  }{
    $\text{\RICE}_{(\BPORDER)~p} \rightarrow$ \WRITE 5 unsigned bits\;
  }
  encode residuals from $partition_{\BPORDER~p}$ with $\text{\RICE}_{(\BPORDER)~p}$\;
}
...
\ForEach{residual \emph{\KwTo}residuals}{
  ...
  $\text{MSB} \rightarrow$ \WUNARY with stop bit 1\;
  $\text{LSB} \rightarrow$ \WRITE (Rice parameter) unsigned bits\;
}
...
\eIf{$\LEN(\AUTOCORRELATIONS) > 1$ \AND \AUTOCORRELATIONS aren't all 0.0}{
  ...
  \eIf(\tcc*[f]{from encoding parameters}){\NOT exhaustive model search}{
    ...
    \For{$\ORDER \leftarrow 1$ \emph{\KwTo}maximum LPC order + 1}{
...
\For{$lag \leftarrow 0$ \emph{\KwTo}maximum LPC order + 1}{
...
\end{tabular}\;
\For{$i \leftarrow 1$ \emph{\KwTo}maximum LPC order}{
...
\For{$i \leftarrow 0$ \emph{\KwTo}maximum LPC order}{
  $order \leftarrow i + 1$\;
...
$(\text{subframe order} - 1) \rightarrow$ \WRITE 5 unsigned bits\;
\eIf{$wasted~BPS > 0$}{
  $1 \rightarrow$ \WRITE 1 unsigned bit\;
  $\text{wasted BPS} - 1 \rightarrow$ \WUNARY with stop bit 1\;
}{
...
$\text{QLP precision} - 1 \rightarrow$ \WRITE 4 unsigned bits\;
$\text{QLP shift needed} \rightarrow$ \WRITE 5 signed bits\;
\For(\tcc*[f]{QLP coefficients}){$i \leftarrow 0$ \emph{\KwTo}subframe order}{
  $\text{QLP coefficient}_i \rightarrow$ \WRITE (QLP precision) signed bits\;
}
\BlankLine
\For(\tcc*[f]{calculate signed residuals}){$i \leftarrow 0$ \emph{\KwTo}sample count - order}{
