...
get \PCMCOUNT and \MDSUM from STREAMINFO metadata block\;
...
\uIf{$\TYPEORDER = 0$}{
  \SAMPLE $\leftarrow$ decode CONSTANT subframe with \BLOCKSIZE, \EFFECTIVEBPS\;
}
\uElseIf{$\TYPEORDER = 1$} {
  \SAMPLE $\leftarrow$ decode VERBATIM subframe with \BLOCKSIZE, \EFFECTIVEBPS\;
}
\uElseIf{$8 \leq \TYPEORDER \leq 12$} {
  $\ORDER \leftarrow \TYPEORDER - 8$\;
  \SAMPLE $\leftarrow$ decode FIXED subframe with \BLOCKSIZE, \EFFECTIVEBPS, \ORDER\;
}
\uElseIf{$32 \leq \TYPEORDER \leq 63$} {
  $\ORDER \leftarrow \TYPEORDER - 31$\;
  \SAMPLE $\leftarrow$ decode LPC subframe with \BLOCKSIZE, \EFFECTIVEBPS, \ORDER\;
}
\Else {
...
\BlankLine
$\RESIDUAL \leftarrow$ read residual block with frame's \BLOCKSIZE and subframe's \ORDER\;
\BlankLine
\Switch{\ORDER}{
  \uCase{0} {
    \For{$i \leftarrow 0$ \emph{\KwTo}\BLOCKSIZE}{
...
$\QLPSHIFT \leftarrow \MAX(\text{\READ 5 signed bits}~,~0)$\footnote{negative shifts are noops in the decoder}\;
...
\For{$p \leftarrow 0$ \emph{\KwTo}$2 ^ {\text{\PORDER}}$}{
  \eIf{p = 0}{
    $\PCOUNT \leftarrow \lfloor\text{block size} \div 2 ^ {\PORDER}\rfloor - \text{\ORDER}$
  }{
    $\PCOUNT \leftarrow \lfloor\text{block size} \div 2 ^ {\PORDER}\rfloor$
  }
  partition's residuals $\leftarrow$ decode partition of size \PCOUNT using \CODING\;
  \BlankLine
...
{\relsize{-1}
\ALGORITHM{the residual block's coding method, the partition's residual count}{decoded signed residual values}
\SetKwData{PCOUNT}{partition residual count}
\SetKwData{CODING}{coding method}
\SetKwData{PORDER}{partition order}
\SetKwData{RICE}{Rice parameter}
...
\uIf{$\text{\CODING} = 0$}{
  $\RICE \leftarrow$ \READ 4 unsigned bits\;
  \If(\tcc*[f]{handle unencoded residuals}){$\RICE = 15$}{
    $\ESCAPE \leftarrow$ \READ 5 unsigned bits\;
    \For{$i \leftarrow 0$ \emph{\KwTo}\PCOUNT}{
      $\text{\RESIDUAL}_i \leftarrow$ \READ (escape code) signed bits\;
    }
    \Return \RESIDUAL\;
  }
}
\uElseIf{$\text{\CODING} = 1$}{
  $\RICE \leftarrow$ \READ 5 unsigned bits\;
  \If(\tcc*[f]{handle unencoded residuals}){$\RICE = 31$}{
    $\ESCAPE \leftarrow$ \READ 5 unsigned bits\;
    \For{$i \leftarrow 0$ \emph{\KwTo}\PCOUNT}{
      $\text{\RESIDUAL}_i \leftarrow$ \READ (escape code) signed bits\;
    }
...
\For{$i \leftarrow 0$ \emph{\KwTo}\PCOUNT} {
  ...
  $\text{\LSB}_i \leftarrow$ \READ (Rice parameter) unsigned bits\;
  $\text{\UNSIGNED}_i \leftarrow \text{MSB}_i \times 2 ^ \text{Rice parameter} + \text{LSB}_i$\;
    \eIf{$(\text{\UNSIGNED}_i \bmod 2) = 0$}{
...
\textsf{paritition order} & $\leftarrow$ & 0 \\
...
\eIf{$\CHANNELCOUNT = 2$ \AND (try mid-side \OR try adaptive mid-side)}{
  \begin{tabular}{rcl}
    ...
    \LEFTS & $\leftarrow$ & $\BUILDSUBFRAME(\text{\CHANNEL}_0~,~\BPS)$ \\
    \RIGHTS & $\leftarrow$ & $\BUILDSUBFRAME(\text{\CHANNEL}_1~,~\BPS)$ \\
    \AVGS & $\leftarrow$ & $\BUILDSUBFRAME(\text{\AVERAGE}~,~\BPS)$ \\
    \DIFFS & $\leftarrow$ & $\BUILDSUBFRAME(\text{\DIFFERENCE}~,~\BPS + 1)$ \\
  ...
  write frame header with channel assigment $\CHANNELCOUNT - 1$\;
  \For{$c \leftarrow 0$ \emph{\KwTo}\CHANNELCOUNT}{
    $\text{\SUBFRAME}_c \leftarrow \BUILDSUBFRAME(\text{\CHANNEL}_c~,~\BPS)$\;
...
$0 \rightarrow$ \WRITE 1 unsigned bit\;
$\FRAMENUMBER \rightarrow$ \WRITE as UTF-8 encoded value\;
\uIf{$\EBLOCKSIZE = 6$}{
...
\eIf{all samples are the same}{
  \Return \CONSTANT using $\text{\SAMPLE}_0$\;
}{
  $\WASTEDBPS \leftarrow$ calculate wasted bits per sample for $\text{\SAMPLE}$\;
  \If{$\WASTEDBPS > 0$}{
    \For{$i \leftarrow 0$ \emph{\KwTo}\BLOCKSIZE}{
  ...
  $\VERBATIM \leftarrow$ build VERBATIM subframe from \SAMPLE\;
  \BlankLine
  $\FIXED \leftarrow$ build FIXED subframe from \SAMPLE\;
  \BlankLine
  \eIf(\tcc*[f]{from encoding parameters}){maximum LPC order $ > 0$}{
    $\LPCPARAMS \leftarrow$ compute best LPC coefficients from \SAMPLE\;
    $\LPC \leftarrow$ build LPC subframe from \LPCPARAMS\;
    \BlankLine
...
write encoded residual block based on $\text{\RESIDUAL}_{\ORDER}$'s signed values\;
\EALGORITHM
}
\begin{figure}[h]
  \includegraphics{figures/flac/fixed.pdf}
...
\For{$\PORDER \leftarrow \text{minimum partition order}$ \emph{\KwTo}(maximum partition order + 1)}{
  \eIf{$(\BLOCKSIZE \bmod 2^{\PORDER}) = 0$}{
    \For{$p \leftarrow 0$ \emph{\KwTo}$2 ^ {\PORDER}$}{
      \eIf{$p = 0$}{
        $\text{\PSIZE}_0 \leftarrow \BLOCKSIZE \div 2 ^ {\PORDER} - \ORDER$\;
      }{
        $\text{\PSIZE}_p \leftarrow \BLOCKSIZE \div 2 ^ {\PORDER}$\;
      }
      place next $\text{\PSIZE}_p$ \RESIDUAL values in $\text{\PARTITION}_{\PORDER~p}$\;
      \BlankLine
      $\text{\RICE}_{\PORDER~p} \leftarrow$ compute best Rice parameter for $\text{\PARTITION}_{\PORDER~p}$\;
      \BlankLine
      $\text{\PSIZE}_{\PORDER~p} \leftarrow$ estimate size of $\text{\PARTITION}_{\PORDER~p}$ encoded with $\text{\RICE}_{\PORDER~p}$\;
    }
  }{
    \BREAK\;
  }
...
\BlankLine
choose \PAORDER such that $\SUM(\PSIZE_{(\PAORDER)})$ is smallest\;
\BlankLine
\tcc{write best residual partition(s) to residual block}
\eIf{$\MAX(\text{\RICE}_{\PAORDER}) > 14$}{
  $\CODING \leftarrow 1$\;
}{
  $\CODING \leftarrow 0$\;
}
$\CODING \rightarrow$ \WRITE 2 unsigned bits\;
$\PAORDER \rightarrow$ \WRITE 4 unsigned bits\;
\For{$p \leftarrow 0$ \emph{\KwTo}$2 ^ {\PAORDER}$} {
  \eIf{$\CODING = 0$}{
    $\text{\RICE}_{(\PAORDER)~p} \rightarrow$ \WRITE 4 unsigned bits\;
  }{
    $\text{\RICE}_{(\PAORDER)~p} \rightarrow$ \WRITE 5 unsigned bits\;
  }
  encode residuals from $\text{\PARTITION}_{(\PAORDER)~p}$ with $\text{\RICE}_{(\PAORDER)~p}$\;
}
...
\SetKwFunction{MIN}{min}
$\PSUM \leftarrow \overset{\PSIZE - 1}{\underset{i = 0}{\sum}} |\text{\RESIDUAL}_i|$\;
$\PARAMETER \leftarrow 0$\;
\While{$\PSIZE \times 2 ^ \text{\PARAMETER} < \PSUM$}{
...
\PSUM $\leftarrow \overset{\PSIZE - 1}{\underset{i = 0}{\sum}} |\text{\RESIDUAL}_i|$\;
\eIf{\PARAMETER $> 0$}{
\Return $4 + ((1 + \PARAMETER) \times \PSIZE) + \left\lfloor\frac{\PSUM}{2 ^ {\PARAMETER - 1}}\right\rfloor - \left\lfloor\frac{\PSIZE}{2}\right\rfloor$\;
}{
\Return $4 + ((1 + \PARAMETER) \times \PSIZE) + (\PSUM \times 2) - \left\lfloor\frac{\PSIZE}{2}\right\rfloor$\;
}
\EALGORITHM
}
...
\SetKwData{MSB}{MSB}
\SetKwData{LSB}{LSB}
\For{$i \leftarrow 0$ \emph{\KwTo}\PSIZE}{
  \eIf{$\text{\RESIDUAL}_i \geq 0$}{
    $\text{\UNSIGNED}_i \leftarrow \text{\RESIDUAL}_i \times 2$\;
  }{
    $\text{\UNSIGNED}_i \leftarrow ((-\text{\RESIDUAL}_i - 1) \times 2) + 1$\;
  }
