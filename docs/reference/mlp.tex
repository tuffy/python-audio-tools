...
\VAR{Bypassed Least-Significan Bits} bit per \VAR{Matrix Parameter} value.
...
\begin{equation*}
\text{LSB Count}_i = \text{Huffman Least-Significant Bits}_c - \text{Quant Step Size}_c
\end{equation*}
We read this number of bits from the stream as
our \VAR{Least-Significant Bits} (LSB) value.
If $\text{LSB Count}_i$ is 0, we read no bits from the stream and our
