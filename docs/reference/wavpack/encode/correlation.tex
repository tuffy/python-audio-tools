...
\subsection{1 Channel Correlation Passes}
\label{wavpack:correlate_channels}
\ALGORITHM{a list of signed samples; correlation terms, deltas, weights and samples}{a list of signed residuals; correlation weights and samples for the next block}
\SetKwData{PASS}{pass}
...
\For(\tcc*[f]{perform passes in reverse order}){$i \leftarrow 0$ \emph{\KwTo}\TERMCOUNT}{
  ...
  $\left.\begin{tabular}{r}
    ...
    $\text{\WEIGHT}'_{p~0}$ \\
    $\text{\SAMPLES}'_{p~0}$ \\
  \end{tabular}\right\rbrace \leftarrow$ \hyperref[wavpack:correlate_1ch]{correlate 1 channel}
...
\For(\tcc*[f]{perform passes in reverse order}){$i \leftarrow 0$ \emph{\KwTo}\TERMCOUNT}{
  ...
  $\left.\begin{tabular}{r}
    ...
    $\text{\SAMPLES}'_{p~0}$ \\
    $\text{\SAMPLES}'_{p~1}$ \\
  \end{tabular}\right\rbrace \leftarrow$ \hyperref[wavpack:correlate_2ch]{correlate 2 channel}
  $\left\lbrace\begin{tabular}{l}
  $\text{\PASS}_{i - 1~0}$ \\
  $\text{\PASS}_{i - 1~1}$ \\
  $\text{\TERM}_{i}$ \\
