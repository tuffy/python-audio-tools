...
\subsubsection{AENC Frame}
\begin{figure}[h]
\includegraphics{figures/id3v23/aenc.pdf}
\end{figure}
This frame contains details about the file's encryption.
\VAR{Owner} is a NULL-terminated string of the file's owner.
\VAR{Preview Start} and \VAR{Preview Length} are MPEG frame offsets
indicating unencrypted portions of audio.
\VAR{Encryption Info} is a binary blob of encryption data.
...
\subsubsection{ENCR Frame}
\begin{figure}[h]
\includegraphics{figures/id3v23/encr.pdf}
\end{figure}
This frame is encryption method registration.
...
\subsubsection{IPLS Frame}
\begin{figure}[h]
\includegraphics{figures/id3v23/ipls.pdf}
\end{figure}
This frame contains a list of people involved in the file's creation.
Each string is encoded as a Latin-1 or UCS-2 NULL-terminated
string, depending on if \VAR{Encoding} is 0 or 1, respectively.

\subsubsection{MCDI Frame}
\begin{figure}[h]
\includegraphics{figures/id3v23/mcdi.pdf}
\end{figure}
This frame contains the binary data from a CD's Table of Contents.
That TOC consists of a 4 byte header followed by 8 bytes per CD track.
Valid \VAR{TRCK} frames are required if this frame is present.

\subsubsection{PRIV Frame}
\begin{figure}[h]
\includegraphics{figures/id3v23/priv.pdf}
\end{figure}
This is a frame for private information.
\VAR{Owner} is a NULL-terminated email address or link.
\VAR{Private Data} is binary data.

\subsubsection{PCNT Frame}
\begin{figure}[h]
\includegraphics{figures/id3v23/pcnt.pdf}
\end{figure}
This frame is a 32-bit counter of the number of times the track
has been played.
According to the spec, additional bytes are added should the counter
...
\subsubsection{POPM Frame}
\begin{figure}[h]
\includegraphics{figures/id3v23/popm.pdf}
\end{figure}
This frame contains a ``popularimeter''.
\VAR{User Email} is a NULL-terminated string of the user's email address.
\VAR{Rating} is a value from 1 (worst) to 255 (best), where 0 is unknown.
