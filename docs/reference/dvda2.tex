...
\For{$s \leftarrow 0$ \emph{\KwTo}\SUBSTREAMCOUNT}{
  $(\text{\CHECKDATA}_s~,~\text{\SUBSTREAMEND}_s) \leftarrow$ \hyperref[mlp:read_substream_info]{read substream info}\;
}
...
\ElseIf{$\SUBSTREAMCOUNT = 2$}{
  \eIf{$\text{\CHECKDATA}_1 = 1$}{
    ...
    \hyperref[mlp:decode_substream]{decode next $(\text{\SUBSTREAMEND}_1 - \text{\SUBSTREAMEND}_0)$ bytes as $\text{\SUBSTREAM}_1$}\;
  }
...
\BlankLine
$(\text{\BYPASSEDLSB}_{s}~,~\text{\RESIDUAL}_s) \leftarrow$ \hyperref[mlp:read_residuals]{read residual data based on $\text{\DECODINGPARAMS}_s$}\;
\BlankLine
