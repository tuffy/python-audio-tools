...
{\relsize{-1}
  ...
  \eIf{$\text{\ORDER}_{s~c} > 0$}{
    ...
    \For{i = 0 \emph{\KwTo}$\text{\ORDER}_{s~c}$}{
...
{\relsize{-1}
  ...
  \eIf{$\text{\ORDER}_{s~c} > 0$}{
    ...
    \eIf{$((\READ~\textnormal{1 unsigned bit}) = 1)$}{
      ...
      \For(\tcc*[f]{read state values in reverse order}){i = $\text{\ORDER}_{s~c}$ \emph{\KwDownTo}0}{
...
\For(\tcc*[f]{generate noise channels}){i = 0 \emph{\KwTo}$\text{\BLOCKSIZE}_s$}{
...
\BlankLine
\For(\tcc*[f]{perform channel rematrixing}){m = 0 \emph{\KwTo}$\text{\MATRIXCOUNT}_s$}{
...
\For{c = 0 \emph{\KwTo}$(\text{\MAXMATRIXCHANNEL}_s + 1)$}{
  \eIf{$\text{\OUTPUTSHIFT}_{s~c} > 0$}{
    \For{i = 0 \emph{\KwTo}$\LEN(\text{\REMATRIXED}_{s~c})$}{
