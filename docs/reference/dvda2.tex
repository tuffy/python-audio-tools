...
\hline
\hyperref[mlp:readchannelparams]{$\textbf{channel parameters}_{s~c}$}
& \hyperref[mlp:readfilterparams]{$\textbf{FIR filter parameters}_{s~c}$} \\
& \hyperref[mlp:readfilterparams]{$\textbf{IIR filter parameters}_{s~c}$} \\
...
\Repeat{$\text{\LAST}_s = 1$}{
  ...
  \For{c = $\text{\MINCHANNEL}_s$ \emph{\KwTo}$\text{\MAXCHANNEL}_s$}{
    $\text{\OUTPUT}_{s~c} \leftarrow \text{\BLOCK}_{s~c}$ appended to $\text{\OUTPUT}_{s~c}$\;
  }
...
\uIf(\tcc*[f]{matrix parameters}){$(\text{\FLAG}_{s~6} = 1)$ \AND $((\READ~\textnormal{1 unsigned bit}) = 1)$}{
  ...
  \For{m = 0 \emph{\KwTo}$\text{\MATRIXCOUNT}_{s}$}{
...
$\;
      ...
      \uElseIf{$\text{\HEADERPRESENT}_s = 1$}{
        $\text{\FIRORDER}_{s~c} \leftarrow \text{\IIRORDER}_{s~c} \leftarrow 0$\;
        $\text{\FIRSHIFT}_{s~c} \leftarrow \text{\IIRSHIFT}_{s~c} \leftarrow 0$\;
        $\text{\FIRCOEFF}_{s~c} \leftarrow \text{\IIRCOEFF}_{s~c} \leftarrow \texttt{[]}$\;
        $\text{\IIRSTATE}_{s~c} \leftarrow \texttt{[]}$\;
        $\text{\HUFFMANOFFSET}_{s~c} \leftarrow 0$\;
        $\text{\CODEBOOK}_{s~c} \leftarrow 0$\;
        $\text{\HUFFMANLSBS}_{s~c} \leftarrow 23$\;
      }
      \Else{$\left.\begin{tabular}{r}
          $\text{\FIRPARAMS}_{s~c}$ \\
          $\text{\IIRPARAMS}_{s~c}$ \\
          $\text{\HUFFMANOFFSET}_{s~c}$ \\
          $\text{\CODEBOOK}_{s~c}$ \\
          $\text{\HUFFMANLSBS}_{s~c}$
        \end{tabular}\right\rbrace \leftarrow $ from previous block's corresponding channel in substream\;
      }
    }
    \Return $\DECODINGPARAMETERS\left\lbrace\begin{tabular}{l}
    \FLAG \\
    \BLOCKSIZE \\
    \MATRIXCOUNT \\
    \MATRIX \\
