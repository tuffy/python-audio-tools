...
\SetKwData{MEANCOUNT}{number of means}
...
\begin{algorithm}[H]
  ...
  $c \leftarrow 0$\tcc*{current channel}
  $o \leftarrow 0$\tcc*{offset}
  $s \leftarrow 0$\tcc*{left shift}
  \BlankLine
  \tcc{process commands}
  \Repeat{$\COMMAND = 4$}{
    ...
    \Switch{\COMMAND}{
      ...
      \uCase(\tcc*[f]{BITSHIFT}){6}{
        $s \leftarrow$ read \texttt{unsigned}(2)\;
      }
      \uCase{7}{QLPC} %%FIXME
      \uCase(\tcc*[f]{ZERO}){8}{
        \For{i = 0 \emph{\KwTo}\BLOCKLENGTH}{
          $\text{\SAMPLES}_{c~i} \leftarrow 0$\;
        }
        $c \leftarrow c + 1$\;
      }
    ...
    \If{$c = \CHANNELS$}{
      ...
      $\SHIFTED \leftarrow$ apply left shift $s$ to \SAMPLES\;
      \Return \SHIFTED\;
      $c \leftarrow 0$\;
    }
  }(\tcc*[f]{QUIT command})
...
{\relsize{-1}
\ALGORITHM{block length, offset $o$}{decoded samples}
\SetKwData{BLOCKLENGTH}{block length}
...
\For{i = 0 \emph{\KwTo}\BLOCKLENGTH}{
  ...
  $\text{\SAMPLES}_{c~i} \leftarrow \text{\RESIDUAL}_i + o$\;
}
