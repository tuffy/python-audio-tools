...
void
alac_compute_best_lpc_coeffs(struct i_array *coeffs,
                             int *shift_needed,
                             ...
                             struct i_array *samples)
{
    ...
    alac_tukey_window(&tukey_window, samples->size, 0.5);
    fa_mul_ia(&windowed_signal, &tukey_window, samples);

    /*compute autocorrelation*/
    fa_init(&autocorrelation_values, MAX_LPC_ORDER + 1);
    alac_compute_autocorrelation(&autocorrelation_values,
                                 &windowed_signal,
                                 MAX_LPC_ORDER + 1);
    ...
    alac_compute_lp_coefficients(&lp_coefficients,
                                 &error_values,
    ...
    lpc_order = alac_compute_best_order(&error_values,
                                        samples->size,
                                        bits_per_sample + 5);

    /*quantize coefficients*/
    ia_reset(coeffs);
    alac_quantize_coefficients(faa_getitem(&lp_coefficients,
                                           lpc_order - 1),
...
int
alac_compute_best_order(struct f_array *error_values,
                        int total_samples,
                        int overhead_bits_per_order)
{
    ...
    for (i = 0, order = 1; i < error_values->size; i++, order++) {
        ...
        bits = alac_compute_expected_bits_per_residual_sample(
...
void
alac_rectangular_window(struct f_array *window, int L)
{
    int n;
...
void
alac_hann_window(struct f_array *window, int L)
{
    int n;
...
void
alac_tukey_window(struct f_array *window, int L, double p)
{
    ...
    alac_rectangular_window(&rect_window, L - hann_length);
    alac_hann_window(&hann_window, hann_length);
...
void
alac_compute_autocorrelation(struct f_array *values,
                             struct f_array *windowed_signal,
                             int max_lpc_order)
{
    int i, j;
...
void
alac_compute_lp_coefficients(struct fa_array *lp_coefficients,
                             struct f_array *error_values,
                             struct f_array *autocorrelation_values,
                             int max_lpc_order)
{
...
double
alac_compute_expected_bits_per_residual_sample(double lpc_error,
                                               double error_scale)
{
    if (lpc_error > 0.0) {
...
void
alac_quantize_coefficients(struct f_array *lp_coefficients,
                           int precision,
