...
static inline void
fade_samples(int samples[], unsigned channels, unsigned index, unsigned total)
{
    double adjustment = index;
    adjustment /= total;

    for (; channels; channels--) {
        samples[0] = (int)(lround(adjustment * samples[0]));
        samples += 1;
    }
}
...
{
    pcmconverter_FadeInReader *self;

    self = (pcmconverter_FadeInReader *)type->tp_alloc(type, 0);
                  ...
                  PyObject *args, PyObject *kwds)
{
    ...
    if (total_samples < 1) {
        PyErr_SetString(PyExc_ValueError, "total samples must be > 0");
        return -1;
    } else {
        self->frame_index = 0;
        self->frame_total = total_samples;
    }
...
FadeInReader_read(pcmconverter_FadeInReader *self, PyObject *args)
{
    ...
    for (frame = 0; frame < frames_read; frame++) {
        fade_samples(framelist->samples + (frame * channels),
                     channels,
                     self->frame_index,
                     self->frame_total);

        if (self->frame_index < self->frame_total) {
            self->frame_index += 1;
        }
    }

    /*return faded FrameList object*/
    return (PyObject*)framelist;
}
...
{
    pcmconverter_FadeOutReader *self;

    self = (pcmconverter_FadeOutReader *)type->tp_alloc(type, 0);
...
FadeOutReader_read(pcmconverter_FadeOutReader *self, PyObject *args)
{
    ...
    for (frame = 0; frame < frames_read; frame++) {
        fade_samples(framelist->samples + (frame * channels),
                     channels,
                     self->frame_total - self->frame_index,
                     self->frame_total);
...
MOD_INIT(pcmconverter)
{
    ...
    pcmconverter_FadeInReaderType.tp_new = PyType_GenericNew;
    if (PyType_Ready(&pcmconverter_FadeInReaderType) < 0)
        return MOD_ERROR_VAL;

    pcmconverter_FadeOutReaderType.tp_new = PyType_GenericNew;
    if (PyType_Ready(&pcmconverter_FadeOutReaderType) < 0)
        return MOD_ERROR_VAL;
    ...
    Py_INCREF(&pcmconverter_FadeInReaderType);
    PyModule_AddObject(m, "FadeInReader",
                       (PyObject *)&pcmconverter_FadeInReaderType);

    Py_INCREF(&pcmconverter_FadeOutReaderType);
    PyModule_AddObject(m, "FadeOutReader",
                       (PyObject *)&pcmconverter_FadeOutReaderType);
